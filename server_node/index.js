...
mongoose.connect(config.mongoURI, { // 몽고디비 연결 주소를 넣어주도록 한다.
    useNewUrlParser: true, useUnifiedTopology: true //옵션 -> 에러를 막아준다.
...
// Index
app.get('/article/post', async function(req, res){ // 1: await 키워드를 사용하는데, await 키워드를 사용하는 함수는 반드시 async 키워드를 function 키워드 앞에 붙여야 함
...
app.get("/article/api/users/auth", auth ,(req, res) => { //auth 라는 middleware를 설치한다.
